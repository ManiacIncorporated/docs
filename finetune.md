# Maniac Finetune: The AI Container Factory

**Maniac Finetune** is the heart of the Maniac platform. It is responsible for creating the **AI Containers** that power your applications. This service takes your raw data and a base model, and produces a portable, task-specific package that can be deployed across any model in the Maniac ecosystem.
...
4.  **Packaging**: Bundling the LoRA, system prompts, and other metadata into a standardized AI Container format.

## Architecture

Maniac Finetune is designed as a modular pipeline that orchestrates the creation of AI Containers. The key components include:

-   `finetune_app.py`: The main entry point for the finetuning service. It likely exposes an API to trigger new training jobs and manage the lifecycle of AI Containers.
...
-   `supabase_utils.py`: This utility suggests that Maniac Finetune integrates with a Supabase backend to store metadata about the training jobs, model artifacts, and evaluation results. This provides a persistent record of all AI Containers created by the system.
